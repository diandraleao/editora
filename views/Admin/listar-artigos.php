...
<html>
	<head>
		<meta charset="utf-8" />
		<title>Listar artigos | Editora Paralogike</title>
		...
		<script type="text/javascript">
		    ...
		    function send_data(campo){
				...
				var url = "<?= base_url(); ?>admin/deletarartigo/" + $(campo).attr("data-id-excluir");//.getAttribute('data-id-excluir');

				xmlhttp.onreadystatechange = function() {
				    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				        //var js = JSON.parse(xmlhttp.responseText);
				   		//alert(js);
				   		$(campo).parent().parent().fadeOut(500, function(){
				   			$(this).remove();
				   		});
				      			...
				      			<tbody>
								    ...
								    <?php 
										...
										foreach($publicacoes as $publicacao){
											...
											echo "<tr> <td>" . $publicacao['id_publicacao'] . "</td> <td>" . $publicacao['ds_titulo_publicacao'] . "</td> <td>" . $publicacao['dt_publicacao'] . "</td> <td><span class='excluir' onclick='send_data(this)' data-id-excluir='" . $publicacao['id_publicacao'] . "'>Excluir</span></td> </tr>";
